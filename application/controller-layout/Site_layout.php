...
abstract class Site_layout extends Base_layout {
    ...
    public function convert_vietnamese_string($str) {
        ...
        $str = preg_replace("/(Ỳ|Ý|Ỵ|Ỷ|Ỹ)/", "Y", $str);
        $str = preg_replace("/(Đ)/", "D", $str);
        return str_replace(" ", "_", str_replace("&*#39;", "", $str));
    }

    private function numInWords($num = FALSE) {
        $nwords = array(
            0 => 'không',
            1 => 'một',
            2 => 'hai',
    ...
    public function read_money_for_vietnamese($num) {
        $str = str_replace("mươi năm", "mươi lăm", str_replace("mươi một", "mươi mốt", $this->numInWords($num)));
